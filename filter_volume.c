/******************************************************************************
 *
 *       Filename:  filter_volume.c
 *
 *    Description:  test ffmpeg filter volume
 *    读取文件更改声音后,保存文件
 *
 *        Version:  1.0
 *        Created:  2021年11月19日 13时12分15秒
 *       Revision:  none
 *       Compiler:  gcc
 *
 *         Author:  yangkun (yk)
...
static int process_all(){
    ...
    AVFormatContext* input_format_contexts[1];
    AVCodecContext* input_codec_contexts[1];
    input_format_contexts[0] = input_format_context_0;
    input_codec_contexts[0] = input_codec_context_0;
    
    AVFilterContext* buffer_contexts[1];
    buffer_contexts[0] = src0;
	
    int input_finished[1];
    input_finished[0] = 0;
