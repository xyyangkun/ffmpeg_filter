...
int test_getinfo(char *path)
{
	...
	ret = mp4_get_info(path, &video_type, &audio_type, &duration);

	if(video_type == 0)
		printf("vidoe_type:novideo or not support\n");
	else if(video_type == 1)
		printf("vidoe_type: h264\n");
	else if(video_type == 2)
		printf("vidoe_type: h265\n");
	else
		printf("vidoe_type:other novideo or not support\n");

	if(audio_type == 0)
		printf("audio_type:not support or no audio\n");
	else if(audio_type == 1)
		printf("audio_type: aac type\n");
	else
		printf("audio_type:not support or no audio\n");
