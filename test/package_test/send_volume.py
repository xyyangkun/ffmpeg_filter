...
def get_v_screen_data():
    ...
    _data += '{0:04x}'.format(socket.htons(0))
    _data += '{0:04x}'.format(socket.htons(1))
    # type
    _data += '03'
...
if __name__ == "__main__":
    ...
    parser.add_option('-p', '--port', dest='port', default=8888, type='int', help='The port to send to')
    parser.add_option('-a', '--addr', dest='ip', default="172.20.2.7", type='string', help='The dest ip to send to')
