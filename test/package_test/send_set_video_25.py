...
# 获取虚拟坐席布局
def get_v_screen_data(type="0101"):
    #_data = '\xab\xa6' # head
    _data = "eba6" # head
    _data += "02"  #group id
    _data += "25"  #cmd id      
...
if __name__ == "__main__":
    ...
    parser.add_option('-t', '--type', dest='type', default="0101", type='string', help='video set type')
    opts, args = parser.parse_args()
    tcp_sock = prepare_sock(opts.ip, opts.port)

    tcp_sock.send(get_v_screen_data(opts.type))
